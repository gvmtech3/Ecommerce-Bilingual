// src/pages/CustomerDashboardPage.jsx - ✅ 100% BILINGUAL
...
function CustomerDashboardPage() {
  ...
  const fetchOrders = async () => {
    ...
    try {
      const mockOrders = [
        { id: '#ORD-001', date: '2026-02-05', items: 3, total: '$267.00', status: 'delivered', products: ['Silk Blouse x2', 'Satin Dress x1'] },
        { id: '#ORD-002', date: '2026-01-28', items: 1, total: '$129.00', status: 'processing', products: ['Cashmere Scarf x1'] },
      ]
      ...
      <div className="flex flex-col lg:flex-row gap-12">
        ...
        <div className="lg:w-1/3 space-y-6 mb-12 lg:mb-0">
          <div className="bg-white p-8 rounded-2xl shadow-lg border border-[#D9A441]/20">
            <h3 className="font-serif text-2xl text-[#13293D] mb-4">
              {t('dashboard.quickStats') || 'Quick Stats'}
            </h3>
            <div className="space-y-4">
              <div className="flex justify-between">
                <span>{t('dashboard.totalOrders') || 'Total Orders'}</span>
                <span className="font-bold text-2xl text-[#D9A441]">{orders.length}</span>
              </div>
              <div className="flex justify-between">
                <span>{t('dashboard.totalSpent') || 'Total Spent'}</span>
          ...
          <div className="bg-white p-6 rounded-2xl shadow-lg border border-[#D9A441]/20">
            <h4 className="font-serif text-xl text-[#13293D] mb-4">
              {t('dashboard.filterOrders') || 'Filter Orders'}
            </h4>
            <div className="grid grid-cols-2 gap-2 text-sm">
              ...
              >
                {t('dashboard.pastMonth') || 'Past Month'}
              </button>
              <button
              ...
              >
                {t('dashboard.pastYear') || 'Past Year'}
              </button>
            </div>
          </div>
        </div>
          ...
          <div className="bg-white rounded-2xl shadow-lg border border-[#D9A441]/20 overflow-hidden">
            <div className="p-8 border-b border-[#D9A441]/20">
              <h2 className="font-serif text-2xl text-[#13293D]">
                {t('dashboard.myOrders', { count: orders.length }) || `My Orders (${orders.length})`}
              </h2>
            </div>
            ...
            <div className="overflow-x-auto">
              <table className="w-full">
                <thead className="bg-[#F6F3F0]">
                  <tr>
                    <th className="p-6 text-left font-semibold text-[#5A5A5A]">
                    ...
                    <tr key={order.id} className="border-t border-[#D9A441]/10 hover:bg-[#F6F3F0]/50">
                      ...
                      <td className="p-6">
                        <span className={`px-3 py-1 rounded-full text-xs font-semibold ${statusColors[order.status]}`}>
                          {t(`dashboard.statuses.${order.status}`) || order.status.toUpperCase()}
                        </span>
                      </td>
                    </tr>
                  ))}
