...
const getGallery = (product) =>
  product?.images?.length ? product.images : [product.imageUrl || image3];
  ...
  return (
    <div className="flex gap-0.5">
      {[1, 2, 3, 4, 5].map((i) => (
        <Star
          key={i}
          className={`${dim} ${i <= rating ? "fill-[#D9A441] text-[#D9A441]" : "text-[#D9A441]/25"}`}
        />
      ))}
    </div>
  );
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <div className="flex items-center gap-1.5">
              ...
              {review.verified && (
                <BadgeCheck
                  className="h-3.5 w-3.5 text-[#D9A441]"
    ...
    <div className="max-w-2xl space-y-5">
      ...
      <div className="flex flex-col gap-4 rounded-2xl border border-[#D9A441]/20 bg-white/60 p-5 sm:flex-row sm:items-center">
        ...
        <div className="flex shrink-0 flex-col items-center text-center sm:pr-5 sm:border-r sm:border-[#D9A441]/20">
          {totalCount > 0 ? (
            <>
              <p className="font-serif text-5xl font-bold leading-none text-[#13293D]">
                {avgRating}
              </p>
            ...
            <>
              <p className="font-serif text-3xl font-bold text-[#13293D]/30">
                —
              </p>
            ...
            {[5, 4, 3, 2, 1].map((star) => {
              ...
              return (
                <div key={star} className="flex items-center gap-2">
                  ...
                  <span className="w-3 text-[10px] text-[#5A5A5A]">
                    {count}
                  </span>
                </div>
              );
      ...
      {totalCount === 0 && (
        <div className="rounded-2xl border border-dashed border-[#D9A441]/25 bg-white/40 px-6 py-10 text-center">
          ...
          <p className="mt-1 text-xs text-[#5A5A5A]">
            {t(
              "review.purchaseToReview",
              "Purchase and receive your order to leave a review.",
            )}
          </p>
        </div>
      )}
...
export default function ProductDetailPage() {
  ...
  // ── Smart Related Products Recommendation ─────────────────────
  let relatedProducts = PRODUCTS.filter(
    (p) => p.categoryId === product?.categoryId && p.id !== id,
  );

  // Add same tag products if not enough
  if (relatedProducts.length < 4) {
    ...
    relatedProducts = [...relatedProducts, ...tagMatches];
  }

  // Add fallback products if still not enough
  if (relatedProducts.length < 4) {
    const fallback = PRODUCTS.filter(
      (p) => p.id !== id && !relatedProducts.some((r) => r.id === p.id),
    );

    relatedProducts = [...relatedProducts, ...fallback];
  }

  // Move out-of-stock to the end
  relatedProducts = relatedProducts.sort((a, b) => b.stock - a.stock);
  ...
  const description =
    lang === "es" ? product.descriptionEs : product.description;
    ...
    <div className="min-h-screen bg-[#F6F3F0]">
      ...
      <div className="mx-auto max-w-6xl px-4 pt-6">
        <nav className="flex items-center gap-2 text-[10px] uppercase tracking-widest text-[#5A5A5A]">
          ...
          <span>/</span>
          <span className="text-[#13293D]">
            {lang === "es" ? category?.nameEs : category?.name}
          </span>
          <span>/</span>
          <span className="max-w-40 truncate font-semibold text-[#13293D]">
            {name}
          </span>
        </nav>
      </div>
      ...
      >
        <div className="grid gap-10 lg:grid-cols-2">
          ...
          <div className="flex flex-col gap-3 sm:flex-row">
            ...
            <div className="relative flex-1 overflow-hidden rounded-3xl border border-[#D9A441]/20 bg-white shadow-sm">
              ...
              {product.tag && (
                <span
                  className="absolute left-4 top-4 rounded-full px-3 py-1 text-[10px] font-bold uppercase tracking-wider shadow"
              ...
              <button
                ...
                className={`absolute right-4 top-4 flex h-9 w-9 items-center justify-center rounded-full border transition-all ${
                  wishlist
                    ...
                    : "border-white/60 bg-white/80 text-[#5A5A5A] hover:text-red-400"
                }`}
              >
                <Heart
                  className={`h-4 w-4 ${wishlist ? "fill-red-400" : ""}`}
                />
              </button>
            </div>
          </div>
          ...
          <div className="flex flex-col gap-5">
            <div>
              ...
              <h1 className="mt-1 font-serif text-3xl leading-tight text-[#13293D] md:text-4xl">
                {name}
              </h1>
              ...
              <div className="mt-2 flex items-center gap-3">
                {reviewSummary.count > 0 ? (
                  <>
                    <StarDisplay
                      rating={Math.round(reviewSummary.avg)}
                      size="md"
                    />
                    <button
            ...
            <div className="flex items-baseline gap-3">
              ...
              {product.stock > 0 && product.stock <= 5 && (
                <span className="rounded-full bg-amber-50 px-2.5 py-1 text-[10px] font-semibold text-amber-600">
                  {t("catalog.onlyLeft", `Only ${product.stock} left`, {
                    count: product.stock,
                  })}
                </span>
              )}
                  ...
                  <button
                    ...
                    className={`h-8 w-8 rounded-full border-2 transition-all ${
                      selectedColor.name === color.name
                        ? "scale-110 border-[#13293D] shadow-md"
            ...
            <div className="flex gap-3">
              <button
                ...
                className={`flex flex-1 items-center justify-center gap-2.5 rounded-2xl py-4 text-xs font-semibold uppercase tracking-widest text-white shadow-md transition-all disabled:opacity-50 ${
                  added
                    ? "bg-green-500"
                    : "bg-[#13293D] hover:bg-[#1a3a55] hover:shadow-lg"
                }`}
              >
                {adding ? (
                  <svg
            ...
            <div className="grid grid-cols-3 gap-2 rounded-2xl border border-[#D9A441]/15 bg-white/60 p-4">
              {[
                {
                  icon: Truck,
                  labelKey: "catalog.freeShipping",
                  def: "Free Shipping",
                },
                {
                  icon: RotateCcw,
                  labelKey: "catalog.easyReturns",
                  def: "30-day Returns",
                },
                {
                  icon: Shield,
                  labelKey: "catalog.securePayment",
                  def: "Secure Payment",
                },
            ...
            {["description", "details", "reviews"].map((tab) => (
              ...
              >
                ...
                {tab === "reviews" && reviewSummary.count > 0 && (
                  <span className="ml-1.5 text-[9px] text-[#D9A441]">
                    ({reviewSummary.count})
                  </span>
                )}
              </button>
              ...
              <div className="max-w-2xl">
                ...
                <ul className="mt-6 space-y-2">
                  {[
                    ...
                    t(
                      "catalog.detail3",
                      "Made in our atelier — small-batch production",
                    ),
                    t("catalog.detail4", "Model is 175cm and wears size S"),
                  ].map((item, i) => (
                    <li
                      key={i}
                      className="flex items-center gap-2.5 text-sm text-[#5A5A5A]"
                    >
              ...
              <div className="max-w-lg">
                <table className="w-full text-sm">
                  <tbody className="divide-y divide-[#D9A441]/15">
                    {[
                      [
                        t("catalog.material", "Material"),
                        t("catalog.materialValue", "100% Mulberry Silk"),
                      ],
                      [
                        t("catalog.weight", "Weight"),
                        t("catalog.weightValue", "16 momme"),
                      ],
                      [
                        t("catalog.origin", "Origin"),
                        t("catalog.originValue", "Made in our atelier"),
                      ],
                      [
                        t("catalog.care", "Care"),
                        t("catalog.careValue", "Dry clean recommended"),
                      ],
                      [
                        t("catalog.stock", "Stock"),
                        product.stock > 0
        ...
        {(() => {
          ...
          return (
            <div className="mt-10 border-t border-[#D9A441]/20 pt-12">
              {/* Header */}
              <div className="mb-7 flex items-center justify-between">
                ...
                <button
                  onClick={() => navigate("/catalog")}
                  className="text-xs font-semibold uppercase tracking-wide text-[#13293D] underline underline-offset-4 hover:text-[#D9A441]"
                >
              ...
              <div className="grid grid-cols-2 gap-5 sm:grid-cols-3 lg:grid-cols-4">
                {relatedProducts.map((related) => {
                  ...
                  const outOfStock = related.stock === 0;

                  return (
                    <div
                      key={related.id}
                      className="group overflow-hidden rounded-2xl border border-[#D9A441]/20 bg-white transition-all hover:-translate-y-1 hover:shadow-lg"
                    >
                      {/* Image */}
                      <div
                        onClick={() => {
                          navigate(`/catalog/${related.id}`);
                          window.scrollTo({ top: 0, behavior: "smooth" });
                        }}
                        className="relative cursor-pointer aspect-4/3 overflow-hidden bg-[#F6F3F0]"
                      >
                        <img
                          src={related.imageUrl || image1}
                          alt={relName}
                        ...
                        {outOfStock && (
                          <div className="absolute inset-0 flex items-center justify-center bg-white/70 backdrop-blur-sm">
                            <span className="rounded-full bg-[#13293D] px-4 py-1 text-[10px] font-semibold uppercase tracking-widest text-white">
                              {t("catalog.outOfStock", "Out of Stock")}
                            </span>
                          </div>
                        )}
                      ...
                      <div className="flex flex-col gap-3 p-4">
                        ...
                        >
                          <p className="line-clamp-1 font-serif text-sm text-[#13293D] group-hover:text-[#D9A441]">
                            {relName}
                          </p>
                        ...
                        <button
                          ...
                          className="w-full rounded-xl bg-[#13293D] py-2 text-[11px] font-semibold uppercase tracking-widest text-white transition-all hover:bg-[#1a3a55] disabled:opacity-40"
                        >
                          {outOfStock
                            ? t("catalog.outOfStock", "Out of Stock")
